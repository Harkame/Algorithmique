...
/**
 * Turtle is a selfcontained class that will allow students to make beautiful
 * turtle graphics with ease.
 *
 * @author Nicholas Seward
 */
@SuppressWarnings("unchecked")
public class Turtle implements Runnable, ActionListener, MouseListener,
	...
	private static HashMap<String, Polygon> shapes; // You can only add. Never
													// remove.
	...
	private void renderLoop() {
		...
		while (running) {
			...
			if (refreshMode == REFRESH_MODE_ANIMATED)
				draw();
			if (!waitUntil(time + 1000000000 / fps))
				fps--;
			else if (fps < 30)
				fps++;
		}
	 ...
	 * applets in the future. For now, it doesn't work because the key and mouse
	 * bindings require reflection and applets think that allowing reflection
	 * would open a security hole. Theoretically in the init method of the
	 * applet you need to place <code>Turtle.startApplet(this);</code>. <b>This
	 * is not currently working.</b>
	 *
	 * @param applet
	 */
	...
	private static void setupBuffering() {
		synchronized (turtleLock) {
			...
			midscreenImage = new BufferedImage(width, height,
					BufferedImage.TYPE_INT_ARGB);
			...
			midscreen.setRenderingHint(RenderingHints.KEY_ANTIALIASING,
					RenderingHints.VALUE_ANTIALIAS_ON);
			onscreen.setRenderingHint(RenderingHints.KEY_ANTIALIASING,
					RenderingHints.VALUE_ANTIALIAS_ON);
	...
	private static void drawTurtleIcon() {
		byte[] imageData = new byte[] { 71, 73, 70, 56, 57, 97, 16, 0, 16, 0,
				-95, 2, 0, 0, -103, 0, 0, -1, 0, -1, -1, -1, -1, -1, -1, 33,
				-7, 4, 1, 10, 0, 2, 0, 44, 0, 0, 0, 0, 16, 0, 16, 0, 0, 2, 44,
				-108, -113, -87, -53, -19, -33, -128, 4, 104, 74, 35, 67, -72,
				34, -21, 11, 124, 27, -90, -107, -109, 72, 117, -91, -71, 110,
				103, -37, 90, -31, -10, -55, -87, 122, -34, 74, 72, -15, 17,
				-56, -127, 8, 33, 5, 0, 59 };
		try {
			BufferedImage tmpicon = ImageIO.read(new ByteArrayInputStream(
					imageData));
	...
	private static void makeShapes() {
		...
		int[] xs = new int[] { 66, 65, 63, 61, 53, 44, 33, 24, 23, 19, 17, 14,
				9, 8, 8, 10, 13, 11, 10, 2, 9, 11, 15, 11, 11, 10, 12, 18, 20,
				22, 23, 26, 35, 44, 53, 61, 62, 64, 66, 71, 77, 78, 77, 76, 72,
				77, 81, 86, 91, 94, 97, 98, 97, 95, 92, 87, 82, 77, 72, 74, 77,
				78, 76, 70 };
		int[] ys = new int[] { 18, 19, 21, 25, 23, 22, 23, 27, 25, 21, 20, 21,
				27, 30, 32, 34, 37, 42, 47, 50, 53, 59, 65, 68, 69, 71, 74, 79,
				80, 80, 78, 74, 77, 78, 77, 75, 79, 81, 82, 81, 76, 73, 71, 69,
				66, 59, 60, 61, 60, 58, 54, 50, 46, 42, 40, 39, 40, 41, 34, 32,
				28, 27, 24, 19 };
	...
	/*
	 * .-./*) .-./*) .-./*) .-./*) .-./*) .-./*) .-./*) .-./*) _/___\/ _/___\/
	 * _/___\/ _/___\/ _/___\/ _/___\/ _/___\/ _/___\/ U U U U U U U U U U U U U
	 * U U U .-./*) .-./*) .-./*) .-./*) .-./*) .-./*) _/___\/ _/___\/ _/___\/
	 * TURTLE CONSTRUCTION_/___\/ _/___\/ _/___\/ U U U U U U U U U U U U .-./*)
	 * .-./*) .-./*) .-./*) .-./*) .-./*) .-./*) .-./*) _/___\/ _/___\/ _/___\/
	 * _/___\/ _/___\/ _/___\/ _/___\/ _/___\/ U U U U U U U U U U U U U U U U
	 */

	/**
	 * This is a internal constuctor that makes a singleton that does the
	 * listening but is not added to the stack of turtles to be rendered. You
	 * don't need to use this outside of the Turtle.java file.
	 *
	 * @param i
	 *            Pass this any integer. It doesn't do anything.
	 */
	 ...
	 * _/___\/ _/___\/ _/___\/ _/___\/ _/___\/ _/___\/ U U U U U U U U U U U U U
	 * U U U .-./*) .-./*) .-./*) .-./*) .-./*) .-./*) _/___\/ _/___\/ _/___\/
	 * STATE MANAGEMENT _/___\/ _/___\/ _/___\/ U U U U U U U U U U U U .-./*)
	 * .-./*) .-./*) .-./*) .-./*) .-./*) .-./*) .-./*) _/___\/ _/___\/ _/___\/
	 * _/___\/ _/___\/ _/___\/ _/___\/ _/___\/ U U U U U U U U U U U U U U U U
	 */
	...
	private long storeCurrentState(boolean animate, boolean isStamp,
			double dotSize, Color dotColor, Font font, String text,
			int justification, Point2D.Double textOffset) {
		...
		synchronized (turtleLock) {
			...
			if (!turtleStates.isEmpty() && turtleStates.lastKey() > time)
				time = turtleStates.lastKey() + 1;
			if (animate)
				time += (long) (speed * 1000000);
			state.set(0, time);
			turtleStates.put(time, state);
			redoStates.clear();
		}
		if (refreshMode == REFRESH_MODE_STATE_CHANGE)
			draw();
		if (refreshMode == REFRESH_MODE_ANIMATED)
			waitUntil(time);
		return time;
	...
	public boolean contains(int x, int y) {
		...
		y1 = _location.y;
		dir1 = _direction;
		m.translate(((x1 - centerX) * scale + width / 2), ((y1 - centerY)
				* (-scale) + height / 2));
		m.scale(scale, scale);
		if (image == null) {
			...
			y = (int) point.y;
			try {
				// System.out.println((new Color(image.getRGB(x,
				// y),true)).getAlpha());
	 ...
	 * _/___\/ _/___\/ _/___\/ _/___\/ _/___\/ _/___\/ U U U U U U U U U U U U U
	 * U U U .-./*) .-./*) .-./*) .-./*) .-./*) .-./*) _/___\/ _/___\/ _/___\/
	 * TURTLE METHODS _/___\/ _/___\/ _/___\/ U U U U U U U U U U U U .-./*)
	 * .-./*) .-./*) .-./*) .-./*) .-./*) .-./*) .-./*) _/___\/ _/___\/ _/___\/
	 * _/___\/ _/___\/ _/___\/ _/___\/ _/___\/ U U U U U U U U U U U U U U U U
	 */
	 ...
	 *
	 * @param direction
	 *            angle counter-clockwise from east
	 * @return state change timestamp
	 */
	public long setDirection(double direction) {
		double a = direction;
		while (this.direction - a > 180)
			a += 360;
		while (this.direction - a < -180)
			a -= 360;
		this.direction = a;
	...
	public double distance(double x, double y) {
		return Math.sqrt((y - location.y) * (y - location.y) + (x - location.x)
				* (x - location.x));
	}
	 ...
	 *            y coordinate
	 * @param direction
	 *            angle counter-clockwise from east in degrees
	 * @return state change timestamp
	 */
	...
	/**
	 * Adds an additional angle to rotation of the turtle's shape when
	 * rendering. This is useful when you need to face a different direction
	 * than the direction you are moving in.
	 *
	 * @param angle
	 *            angle in degrees
	 * @return state change timestamp
	...
	/**
	 * Sets the angle to rotate the turtle's shape when rendering. This is
	 * useful when you need to face a different direction than the direction you
	 * are moving in.
	 *
	 * @param angle
	 *            angle in degrees
	 ...
	 *
	 * @param outlineColor
	 *            Color of the turtle's outlineColor.
	 * @return state change timestamp
	 */
	 ...
	 *
	 * @param shape
	 *            shapename or filename of image
	 * @return state change timestamp
	 */
	 ...
	 */
	public long stamp() {
		long timeStamp = storeCurrentState(true, true, 0, null, null, null, 0,
				null);
		return timeStamp;
	}
	 ...
	 */
	public long dot() {
		long timeStamp = storeCurrentState(true, false, penWidth * 3, penColor,
				null, null, 0, null);
		return timeStamp;
	}
	...
	public long dot(String color) {
		...
		if (c == null)
			c = penColor;
		long timeStamp = storeCurrentState(true, false, penWidth * 3, c, null,
				null, 0, null);
		return timeStamp;
	}
	...
	public long dot(Color color) {
		long timeStamp = storeCurrentState(true, false, penWidth * 3, color,
				null, null, 0, null);
		return timeStamp;
	}
	...
	public long dot(String color, double dotSize) {
		...
		if (c == null)
			c = penColor;
		long timeStamp = storeCurrentState(true, false, dotSize, c, null, null,
				0, null);
		return timeStamp;
	}
	...
	public long dot(Color color, double dotSize) {
		long timeStamp = storeCurrentState(true, false, dotSize, color, null,
				null, 0, null);
		return timeStamp;
	}

	public long write(String text, String fontName, int fontSize,
			int justification, double xOffset, double yOffset) {
		return 0;
	}
	 ...
	 *
	 * @param steps
	 *            the number of state changes to remove
	 */
	public void undo(int steps) {
		for (int i = 0; i < steps; i++)
			rollback();
		lastUpdate = 0;
		if (refreshMode != REFRESH_MODE_ON_DEMAND)
			updateAll();
	}
	 ...
	 *
	 * @param steps
	 *            the number of state changes to restore
	 */
	...
	/**
	 * Clears all the drawing that a turtle has done but all the turtle settings
	 * remain the same. (color, location, direction, shape)
	 */
	public void clear() {
		synchronized (turtleLock) {
			long removeKey = 0;
			TreeMap<Long, ArrayList> copy_turtleStates = (TreeMap<Long, ArrayList>) turtleStates
					.clone();
			for (Map.Entry<Long, ArrayList> entry : copy_turtleStates
					.entrySet()) {
	...
	private void rollback() {
		...
		synchronized (turtleLock) {
			long removeKey = 0;
			long restoreTime = 0;
			for (Map.Entry<Long, ArrayList> entry : turtleStates
	...
	private void rollforward() {
		synchronized (turtleLock) {
			for (Map.Entry<Long, ArrayList> entry : redoStates.entrySet()) {
				...
				long time = entry.getKey();
				if (getStateTurtle(state) == this) {
					turtleStates.put(entry.getKey(),
							redoStates.remove(entry.getKey()));
					restoreState(time);
					return;
				}
			}
	...
	/**
	 * This specifies when the screen gets refreshed. 0(default)=Animated (The
	 * turtle will slide from one state to another without being jerky.) 1=State
	 * Change (The turtle will refresh immediately to the last state. Jerky
	 * motion.) 2=On Demand (The turtle will refresh only when you call
	 * update())
	 *
	 * @param mode
	 *            refresh mode
	...
	/**
	 * This specifies how the background is drawn. 0=The image if present is
	 * stretched to fill the screen. 1=The image is centered on the middle of
	 * the screen and will not scale/pan 2=The image is tiled and will not
	 * scale/pan 3=The image is centered on (0,0) and will scale/pan
	 ...
	 *
	 * @param filename
	 *            filename for a background image
	 */
	...
	private static boolean addMouseBinding(String methodName, Turtle t,
			boolean append, boolean click, boolean repeat) {
		String className = "";
		try {
		...
		if (!append || !mouseBindings.containsKey(t))
			mouseBindings.put(t, new ArrayList<ArrayList>());
	...
	private boolean addKeyBinding(String methodName, String keyText,
			boolean append, boolean repeat) {
		...
		if (!append || !keyBindings.containsKey(keyText))
			keyBindings.put(keyText, new ArrayList<ArrayList>());
		ArrayList binding = new ArrayList();
		binding.add(this);
		binding.add(className);
		binding.add(methodName);
	 ...
	 *
	 * @param methodName
	 *            method to be executed when the key is pressed
	 * @param keyText
	 *            key that triggers the method
	 * @return
	 */
	 ...
	 *            key that triggers the method
	 * @param append
	 *            true if you want to have multiple methods per key
	 * @return
	 */
	 ...
	 *            true if you want to have multiple methods per key
	 * @param repeat
	 *            true if you want call the method every screen refresh
	 * @return
	 */
	public boolean onKey(String methodName, String keyText, boolean append,
			boolean repeat) {
	 ...
	 * @param miny
	 *            bottom y coordinate of box
	 * @param maxx
	 *            right x coordinate of box
	 * @param maxy
	...
	public static void zoomFit() {
		synchronized (turtleLock) {
			...
			if (turtleStates.isEmpty())
				...
				loc = getStateLocation(turtleStates.firstEntry().getValue());
			...
			if (refreshMode != REFRESH_MODE_ANIMATED)
				time = turtleStates.lastKey() + 1;
			for (Map.Entry<Long, ArrayList> entry : turtleStates.headMap(time)
					.entrySet()) {
				ArrayList state = entry.getValue();
				if (!getStateIsPenDown(state))
					continue;
			...
			if (turtleStates.lastKey() > time
					&& getStateSpeed(turtleStates.lastEntry().getValue()) > 0) {
				double percent = 1 - (turtleStates.lastKey() - time)
			...
			zoom(minx - shapeMax / 2, miny - shapeMax / 2, maxx + shapeMax / 2,
					maxy + shapeMax / 2);
		}
	}
	...
	private static void draw() {
		synchronized (turtleLock) {
			...
			if (renderTime < turtleStates.lastKey()) {
				animatedTurtle = getStateTurtle(turtleStates.ceilingEntry(
						renderTime).getValue());
				...
				if (animatedTurtle._speed > 0) {
					percent = 1
							...
							/ animatedTurtle._speed / 1000000.0;
				} else
					percent = 1;
				if (percent < 0)
					percent = 0;
			...
			for (Turtle t : turtles) {
				if (t == animatedTurtle) {
					...
					if (t._dotSize > 0)
						t.drawDot(percent, midscreen);
					if (t.isVisible)
						t.drawStamp(percent, midscreen, false);
					if (t._isStamp)
						t.drawStamp(percent, midscreen, true);
	...
	private void drawLine(double percent, Graphics2D g) {
		...
		g.setStroke(new BasicStroke((float) (scale * _penWidth),
		...
		if (__location != null && !__location.equals(_location)) {
			...
			// g.draw(new Line2D.Double((x1-centerX)*scale+width/2,
			// (y1-centerY)*(-scale)+height/2, (x2-centerX)*scale+width/2,
			// (y2-centerY)*(-scale)+height/2));
			g.drawLine((int) ((x1 - centerX) * scale + width / 2),
	...
	private void drawStamp(double percent, Graphics2D g, boolean isStamp) {
		if (_location == null)
			return;
		AffineTransform originalTransform = (AffineTransform) g.getTransform()
				.clone();
		...
		m.translate(((x1 - centerX) * scale + width / 2), ((y1 - centerY)
				* (-scale) + height / 2));
		if (isStamp)
			m.scale(scale * percent, scale * percent);
		else
			m.scale(scale, scale);
		if (_image == null) {
			// _outlineWidth=0.0;
			...
			if (_outlineWidth > 0) {
				g.setStroke(new BasicStroke((float) (_outlineWidth * scale),
						BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
	...
	private void drawDot(double percent, Graphics2D g) {
		AffineTransform originalTransform = (AffineTransform) g.getTransform()
				.clone();
		AffineTransform m = g.getTransform();
		m.translate(((_location.x - centerX) * scale + width / 2),
	...
	private static void drawBackground(Graphics2D g) {
		...
		if (backgroundMode == BACKGROUND_MODE_CENTER) {
			offscreen.drawImage(backgroundImage, (width - w) / 2,
					(height - h) / 2, w, h, null);
			...
			offscreen.drawImage(backgroundImage, (int) ((-w / 2 - centerX)
					* scale + width / 2),
			...
			for (double x = ((int) (left / w) - 1) * w; x <= right; x += w)
				for (double y = ((int) (bottom / h)) * h; y <= top + h; y += h)
					offscreen.drawImage(backgroundImage, (int) ((x - centerX)
							* scale + width / 2), (int) ((y - centerY)
							* (-scale) + height / 2),
	...
	private void drawCrossHairs(double percent, Graphics2D g) {
		...
		int period = 50;
		int r = (int) (Math.sqrt(shapeWidth * shapeWidth + shapeHeight
				* shapeHeight)
				* f / 2);
		g.setColor(new Color(255, 255, 255));
		g.setStroke(new BasicStroke((float) (6 * f), BasicStroke.CAP_ROUND,
				BasicStroke.JOIN_ROUND));
		g.drawOval(-r, -r, 2 * r, 2 * r);
		r += f;
		for (int i = 0; i < 4; i++)
			g.drawLine(
					(int) (r * Math.cos(Math.PI / 2 * i + 2 * Math.PI * time
							/ period)),
					(int) (r * Math.sin(Math.PI / 2 * i + 2 * Math.PI * time
							/ period)),
					(int) ((r + r / 5) * Math.cos(Math.PI / 2 * i + 2 * Math.PI
							* time / period)),
					(int) ((r + r / 5) * Math.sin(Math.PI / 2 * i + 2 * Math.PI
							* time / period)));
		r -= f;
		g.setColor(new Color(0, 0, 0));
		g.setStroke(new BasicStroke((float) (3 * f), BasicStroke.CAP_ROUND,
				BasicStroke.JOIN_ROUND));
		g.drawOval(-r, -r, 2 * r, 2 * r);
		r += f;
		for (int i = 0; i < 4; i++)
	...
	/**
	 * Changes the size of the canvas effectively changing the size of the
	 * window.
	 *
	 * @param width
	 *            width of the canvas
	...
	private static void save(File file) {
		...
		BufferedImage rgbBuffer = new BufferedImage(newCM, newRaster, false,
				null);
		try {
			String suffix = file.getName().substring(
					file.getName().lastIndexOf('.') + 1);
			if (!ImageIO.write(rgbBuffer, suffix, file))
				throw new Exception("Didn't save file.");
		} catch (Exception e) {
			file.delete();
			JOptionPane.showMessageDialog(window,
					"Sorry! We can not process your request at this time.",
					"Image Save Failed", JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	public static void main(String[] a) {
		...
		/*
		 * for(int i=0;i<360;i++) { bob.forward(i*1.25); bob.left(90.25); }
		 */
		/*
		 * If you don't know what a for loop is yet this is equivalent to
		 * repeating the middle 4 lines 5 times in a row.
		 */
		Turtle bob = new Turtle();
		bgcolor("lightblue");
		bob.penColor("red");
		...
		for (int i = 0; i < 200; i++) {
			...
			bob.left(5);
			if (i % 10 == 0)
				bob.dot("orange");// Draws dots when i is a multiple of 10.
		}
	}
	...
	public void actionPerformed(ActionEvent e) {
		if (((JMenuItem) e.getSource()).getText().equals("Save...")) {
			...
			chooser.setFileFilter(new FileNameExtensionFilter(
					"Image (*.jpg, *.jpeg, *.gif, *.bmp, *.png)", "jpg", "png",
					"jpeg", "bmp", "gif"));
		...
		for (String keyText : keysDownCopy) {
			if (keyBindings.containsKey(keyText)) {
				for (ArrayList binding : keyBindings.get(keyText)) {
					...
					if (!repeat && processedKeys.contains(keyText))
						break;
					...
					try {
						...
						try {
							Class cls = Class.forName(className);
							Object clsInstance = (Object) cls.newInstance();
							Method m = clsInstance.getClass().getMethod(
							...
							try {
								...
								System.out.println("KeyBinding for " + keyText
										+ " has failed.");
								e1.printStackTrace();
								e2.printStackTrace();
								e3.printStackTrace();
	...
	/*
	 * public static double screenX(double canvasX) { return
	 * (canvasX-centerX)*scale+width/2.0; } public static double screenY(double
	 * canvasY) { return (canvasY-centerY)*scale+height/2.0; }
	 */
}
